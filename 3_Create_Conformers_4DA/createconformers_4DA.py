...
# Create the violin plot on the axes
violin_parts = ax_violin.violinplot(data, showmedians=True)

# Set axis labels for the violin plot
ax_violin.set_xlabel('Type')
ax_violin.set_ylabel('N-N distance (Angstrom)')

# Set custom x-tick labels for each violin
ax_violin.set_xticks([1, 2, 3, 4])
ax_violin.set_xticklabels(['DistA', 'DistB', 'DistC', 'DistD'])
...
for i, median in enumerate(medians, start=1):
    ax_violin.text(i, median + 0.05, f'Median: {median:.2f}', horizontalalignment='center', verticalalignment='bottom', color='black')

# Save the violin plot as a PNG file
fig_violin.savefig('violin_plot_5.png', dpi=300, bbox_inches='tight')
...
violin_parts = ax_violin.violinplot([alldistances, alldistances, alldistances, alldistances], positions=[0,1,2,3], showmedians=False)
...
# Overlay data points
ax_violin.scatter(0 + jitter_A, data_A, color=color_A, label='A', alpha=0.7)
ax_violin.scatter(0 + jitter_B, data_B, color=color_B, label='B', alpha=0.7)
ax_violin.scatter(0 + jitter_C, data_C, color=color_C, label='C', alpha=0.7)

# Separate points for each violin
ax_violin.scatter(1 + jitter_A[:4], data_A_1, color='black', alpha=0.7, marker='s')
ax_violin.scatter(1 + jitter_A[4:8], data_A_2, color='black', alpha=0.7, marker='v')
ax_violin.scatter(1 + jitter_A[8:], data_A_3, color='black', alpha=0.7, marker='*')

ax_violin.scatter(2 + jitter_B[:4], data_B_1, color='black', alpha=0.7, marker='s')
ax_violin.scatter(2 + jitter_B[4:8], data_B_2, color='black', alpha=0.7, marker='v')
ax_violin.scatter(2 + jitter_B[8:], data_B_3, color='black', alpha=0.7, marker='*')

ax_violin.scatter(3 + jitter_C[:4], data_C_a_1, color='black', alpha=0.7, marker='s', label = 'Crystal1') 
ax_violin.scatter(3 + jitter_C[4:8], data_C_a_2, color='black', alpha=0.7, marker='v')
ax_violin.scatter(3 + jitter_C[8:12], data_C_a_3, color='black', alpha=0.7, marker='*')
ax_violin.scatter(3 + jitter_C[12:16], data_C_b_1, color='white', edgecolor='black', alpha=0.7, marker='s', label = 'Crystal2')
ax_violin.scatter(3 + jitter_C[16:20], data_C_b_2, color='white', edgecolor='black', alpha=0.7, marker='v')
ax_violin.scatter(3 + jitter_C[20:24], data_C_b_3, color='white', edgecolor='black', alpha=0.7, marker='*')
ax_violin.scatter(3 + jitter_C[24:28], data_C_c_1, color='black', edgecolor='white', alpha=0.7, marker='s', label = 'Crystal3')
ax_violin.scatter(3 + jitter_C[28:32], data_C_c_2, color='black', edgecolor='white', alpha=0.7, marker='v')
ax_violin.scatter(3 + jitter_C[32:], data_C_c_3, color='black', edgecolor='white', alpha=0.7, marker='*')

ax_violin.set_xlabel('Type')
ax_violin.set_ylabel('N-N distance (Angstrom)')
ax_violin.set_xticks([0,1,2,3])
ax_violin.set_xticklabels(['All', 'A', 'B', 'C'])
ax_violin.legend()
fig_violin.savefig('violin_plot_with datapoints_5.png', dpi=300, bbox_inches='tight')
...
index = np.arange(len(distancesa))  # Assuming all distances arrays have the same length
ax_line.plot(index, distancesa, label='DistA', color='blue')
ax_line.plot(index, distancesb, label='DistB', color='orange')
ax_line.plot(index, distancesc, label='DistC', color='green')
ax_line.plot(index, distancesd, label='DistD', color='red')

# Set axis labels for the line plot
ax_line.set_xlabel('Index')
ax_line.set_ylabel('N-N distance (Angstrom)')

# Add a legend to the line plot
ax_line.legend()

# Save the line plot as a PNG file
fig_line.savefig('line_plot_5.png', dpi=300, bbox_inches='tight')
plt.close(fig_line)
